...
<div class="page-title">
    <div class="row">
        ...
        <div class="col-12 col-md-6 text-end mt-0 d-flex justify-content-end align-items-start">
            <a href="{{route('transaksi-kas.index')}}" class="btn icon icon-left btn-light me-2"><i class="bi bi-arrow-left-square"></i> Kembali</a>
            @if ($data->cancel=='N')
                @can('transaksi kas-delete')
                <button class="btn icon icon-left btn-danger me-2 delete" data-url="{{route('transaksi-kas.index')}}" data-id="{{encrypt0($data->id)}}"><i class="bi bi-trash"></i> Batal</button>
                @endcan
            @endif
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownCreate" style="">
                    <a class="dropdown-item text-success fw-semibold" href="{{route('transaksi-kas.create')}}?jenis=Masuk"><i class="bi bi-cart-plus fs-4 me-2"></i> Item Masuk</a>
                    <a class="dropdown-item text-danger fw-semibold" href="{{route('transaksi-kas.create')}}?jenis=Keluar"><i class="bi bi-cart-dash fs-4 me-2"></i> Item Keluar</a>
                    <a class="dropdown-item text-primary fw-semibold" href="{{route('transaksi-kas.create')}}?jenis=Transfer"> <i class="bi bi-cart fs-4 me-2"></i> Tranfer Item</a>
                </div>
            </div>
            @endcan
        </div>
...
<script src="{{asset('app/assets/pages/keuangan/transaksi_kas_detail.js')}}?v={{identity()['assets_version']}}"></script>
@endsection