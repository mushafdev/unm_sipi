<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Login - {{identity()['singkat']}}</title>
    
    
    
    <link rel="shortcut icon" href="{{asset('app/assets/compiled/svg/favicon.svg')}}" type="image/x-icon">
    <link rel="shortcut icon" href="data:image/png;base64," type="image/png">
    <link rel="stylesheet" href="{{asset('app/assets/compiled/css/app.css')}}">
    <link rel="stylesheet" href="{{asset('app/assets/compiled/css/cst.css')}}">
    <link rel="stylesheet" href="{{asset('app/assets/compiled/css/auth-image.css')}}">
</head>

<body>
    <script src="{{asset('app/assets/static/js/initTheme.js')}}"></script>
    <div id="auth">     
        <div class="container body-container">
            <div class="row login-page">
                <div class="row m-auto">    
                      
                    <div class="col-12 text-center mb-4">
                        <img src="{{asset('app/assets/static/images/logo/yps-logo.png')}}" width="80px">
                        <img src="{{asset('app/assets/static/images/logo/vale-logo.png')}}" width="120px">
                    </div>
                    <div class="col-12">
                        <div class="card m-0 p-0 shadow">
                            <div class="card-body row p-0 justify-content-center">
                                <div class="card-image-login d-none d-md-block b-cover" style="width:57.5%;">
                                    @include('u.component.login_left')
                                </div>
                                <div class="col-12 col-md-5 p-5 bg-white">
                                    <div class="auth-logo">
                                        {{-- <a href="#"><img src="{{asset('app/assets/static/images/logo/logo.svg')}}" alt="Logo"></a> --}}

                                        <h2 class="mb-0 text-warning font-bold">CBT<span class="text-primary">System</span></h2>
                                    </div>
                                    <h3 class="auth-title"></h3>
                                    <p class="auth-subtitle mb-3">Hello, <span class="font-bold">Silahkan Login</span></p>
                                    
                                    @if (session()->has('success'))
                                        <div class="alert alert-light-success color-success"><i class="bi bi-check-circle"></i> 
                                            {{ session('success') }}</div>
                                    @endif
                                    @if ($errors->has('error_message'))
                                        <div class="alert alert-light-danger color-danger"><i class="bi bi-x-lg"></i> 
                                            {{ $errors->first('error_message') }}</div>
                                    @endif
                                    <form action="{{route('login.authenticate')}}" method="POST" data-reload-captcha="{{route('login.reload_captcha')}}" id="login" autocomplete="false">
                                        {{ csrf_field() }}
                                        <div class="form-group position-relative has-icon-left mb-4">
                                            <input type="text" class="form-control form-control-lg" name="username" placeholder="Username atau Email" value="{{old('username')}}">
                                            <div class="form-control-icon">
                                                <i class="bi bi-person"></i>
                                            </div>
                                            @if($errors->has('username'))
                                                <p class="text-danger">{{ $errors->first('username') }}</p>
                                            @endif
                                        </div>
                                        <div class="form-group position-relative has-icon-left mb-4">
                                            <div class="d-flex">
                                                <input type="password" class="form-control form-control-lg" id="password" name="password" placeholder="Password">
                                                <div class="form-control-icon">
                                                    <i class="bi bi-shield-lock"></i>
                                                </div>
                                                <button class="btn btn-primary show-pass" type="button"><i class="bi bi-eye-slash"></i></button>
                                                
                                            </div>
                                            @if($errors->has('password'))
                                                <p class="text-danger">{{ $errors->first('password') }}</p>
                                            @endif
                                            @if ($message = Session::get('error'))
                                            <p class="text-danger">{{ $message }}</p>
                                            @endif
                                        </div>
                                        <div class="form-group position-relative has-icon-left mb-4">
                                            <input type="text" class="form-control form-control-lg" name="pin" placeholder="PIN Sesi" value="{{old('pin')}}">
                                            <div class="form-control-icon">
                                                <i class="bi bi-key"></i>
                                            </div>
                                            @if($errors->has('pin'))
                                                <p class="text-danger">{{ $errors->first('pin') }}</p>
                                            @endif
                                        </div>
                                        <div class="form-group position-relative mb-4 row">
                                            <div class="d-flex">
                                                <span><img class="img-captcha" src="{{captcha_src()}}"></span>
                                                <button class="btn btn-primary reload-cpt" type="button"><i class="bi bi-arrow-repeat"></i></button>
                                                <input type="text" class="form-control ms-2" name="captcha" placeholder="Enter Captcha"> 
                                            </div>
                                            @if($errors->has('captcha'))
                                            <p class="text-danger mb-0">{{ $errors->first('captcha') }}</p>
                                            @endif
                                        </div>
                                        <button class="btn btn-warning btn-block btn-lg shadow-lg mt-3">Log in</button>
                                    </form>
                                    <div class="text-center mt-3 text-md fs-6">
                                        <p class="text-gray-600">Support By <a href="#" class="font-bold text-primary">{{identity()['support']}}</a>.</p>
                                        {{-- <p class="text-gray-600">Belum punya akun? <a href="{{route('registrasi')}}" class="font-bold">Daftar Sekarang</a>.</p>
                                        <p><a class="font-bold" href="{{route('login.forgot_password')}}">Lupa password?</a>.</p> --}}
                                    </div>
                                </div>
                            </div>
                        </div>
                        
                    </div>
                </div>
                
            </div>
        </div>
    </div>
    <script src="{{asset('app/assets/extensions/jquery/jquery.min.js')}}"></script>
    <script src="{{asset('app/assets/pages/login.js')}}"></script>
</body>

</html>