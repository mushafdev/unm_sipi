                ...
                <div id="auth-left">
                    ...
                    @if (session()->has('status'))
                        <div class="alert alert-light-success color-success"><i class="bi bi-check-circle"></i> 
                            {{ session('status') }}</div>
                    @endif
                    <form action="{{route('login.forgot_password')}}" method="POST" data-reload-captcha="{{route('login.reload_captcha')}}" id="login" autocomplete="false">
                        {{ csrf_field() }}
                        
                        <div class="form-group position-relative has-icon-left mb-4">
                            <input type="email" class="form-control form-control-lg" name="email" placeholder="Masukka email terdaftar" value="{{old('email')}}" required>
