...
let statusPendaftaranVal = localStorage.getItem("filterStatusPendaftaran") || "";

if (tglDaftarVal) {
    $('#tgl_daftar').val(tglDaftarVal);
}
if (jenisLayananVal) {
    $('#jenis-layanan').val(jenisLayananVal);
}
if (statusPendaftaranVal) {
    $('.status-pendaftaran').removeClass('active');
    $('.status-pendaftaran[data-id="'+statusPendaftaranVal+'"]').addClass('active');
}
...
table = $('#table').DataTable({
    ...
    ajax: {
        ...
        "data": function(d) { 
            ...
            d.status_pendaftaran = $('.status-pendaftaran.active').data('id');             
        },
...
$(document).on('change', '.form-filter', function (e) {
    ...
    if(filter == 'jenis-layanan'){
        localStorage.setItem("filterJenisLayanan", $(this).val());
    }
    
    table.ajax.reload(null, true);
});

$(document).on('click', '.status-pendaftaran', function (e) {
    e.preventDefault();
    localStorage.setItem("filterStatusPendaftaran", $(this).data('id'));
    table.ajax.reload(null, true);
});
